...
Also creates the local branch of each forward-port of mine waiting on me: a conflict, a red CI,
or a chain tip nobody gave r+ yet. Mergebot merges the other ones by itself.
...
def gh(*args):
    ...
    if res.returncode:
        print(f"[yellow]Forward-ports skipped[/yellow]: {res.stderr.strip()}")
        return None
    return res.stdout
...
def get_forward_ports_to_handle():
    ...
    search = " ".join(
        [f"is:pr is:open author:{FW_BOT} mentions:{login.strip()}"]
        + [f"repo:{github_repo}" for github_repo in repo_by_github_repo],
    )
    query = (
        f"{{ search(query: {json.dumps(search)}, type: ISSUE, first: 100) {{ nodes {{ "
    ...
    if not (out := gh("api", "graphql", "-f", f"query={query}")):
        return {}
    heads_by_repo = {}
    for pr in json.loads(out)["data"]["search"]["nodes"]:
        head = pr["headRefName"]
        if f"{BUNDLE_SUFFIX}-" in head and head.endswith("-fw") and needs_me(pr):
            repo = repo_by_github_repo[pr["repository"]["nameWithOwner"]]
            heads_by_repo.setdefault(repo, []).append(head)
    return heads_by_repo


forward_ports_by_repo = get_forward_ports_to_handle()
...
def handle_repo_remote(runner: UtilsRunner, repo, remote, branch_r):
    ...
    if dev:
        ...
        new_forward_ports = [
            head for head in forward_ports_by_repo.get(repo, []) if head not in to_fetch
        ]
        to_fetch += new_forward_ports
    else:
